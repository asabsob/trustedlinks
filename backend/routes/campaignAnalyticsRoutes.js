...
export default function CampaignAnalytics({ lang = "en" }) {
  ...
  async function loadAnalytics() {
    try {
      ...
      const [campaignsRes, participantsRes, platformRes] =
        await Promise.all([
          fetch(`${API_BASE}/api/campaign/campaigns`, { headers }),
          fetch(`${API_BASE}/api/campaign/participants`, { headers }),
          fetch(`${API_BASE}/api/platform/analytics/search-demand`, {
            headers,
          }),
        ]);
      ...
      if (!participantsRes.ok) {
        throw new Error(
          participantsData.error || "Failed to load participants"
        );
      }

      if (!platformRes.ok) {
        throw new Error(
          platformData.error || "Failed to load platform analytics"
        );
      }
    ...
    <div style={pageStyle(isAr)}>
      <section style={heroStyle}>
        ...
        <p style={heroSubtitle}>
          {t(
            "Measure campaign activation, budget usage, demand, and platform performance.",
            "قياس تفعيل الحملات واستخدام الميزانية والطلب وأداء المنصة."
          )}
        </p>
      </section>
      ...
      <section style={panelStyle}>
        <h2 style={panelTitle}>
          {t("Platform Demand Analytics", "تحليلات الطلب على المنصة")}
        </h2>

        <div style={statsGrid}>
          <MetricCard
            title={t("Total Searches", "إجمالي عمليات البحث")}
            value={platform?.overview?.totalSearches || 0}
          />

          <MetricCard
            title={t("Unique Search Users", "عدد مستخدمي البحث")}
            value={platform?.overview?.uniqueUsers || 0}
          />

          <MetricCard
            title={t("No Result Searches", "عمليات بدون نتائج")}
            value={platform?.overview?.noResults || 0}
            color="#dc2626"
          />
        </div>
      </section>
      ...
      <section style={gridTwo}>
        <div style={panelStyle}>
          <h2 style={panelTitle}>{t("Top Search Keywords", "أكثر الكلمات بحثًا")}</h2>

          <SimpleList
            items={platform?.topKeywords || []}
            labelKey="query"
            valueKey="count"
            emptyText={t("No keywords yet", "لا توجد كلمات بعد")}
          />
        </div>

        <div style={panelStyle}>
          <h2 style={panelTitle}>{t("Search Intent Types", "أنواع نية البحث")}</h2>

          <SimpleList
            items={platform?.topIntents || []}
            labelKey="intent"
            valueKey="count"
            emptyText={t("No intent data yet", "لا توجد بيانات بعد")}
          />
        </div>
      </section>
      ...
      <section style={panelStyle}>
        <h2 style={panelTitle}>
          {t("Top Appearing Businesses", "أكثر الشركات ظهورًا")}
        </h2>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={tableStyle}>
            <thead>
              <tr>
                <th style={thStyle(isAr)}>{t("Business", "الشركة")}</th>
                <th style={thStyle(isAr)}>{t("Views", "الظهور")}</th>
                <th style={thStyle(isAr)}>{t("WhatsApp Leads", "طلبات واتساب")}</th>
              </tr>
            </thead>

            <tbody>
              {(platform?.topBusinesses || []).length === 0 ? (
                <tr>
                  <td colSpan="3" style={emptyTd}>
                    {t("No business data yet", "لا توجد بيانات شركات بعد")}
                  </td>
                </tr>
              ) : (
                platform.topBusinesses.map((b) => (
                  <tr key={b.business_id}>
                    <td style={tdStyle}>{isAr ? b.name_ar || b.name : b.name}</td>
                    <td style={tdStyle}>{b.views || 0}</td>
                    <td style={tdStyle}>{b.whatsapp || 0}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <section style={gridTwo}>
        <div style={panelStyle}>
          ...
          <div style={progressText}>{analytics.usageRate.toFixed(1)}%</div>
                ...
                campaigns.map((c) => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td style={tdStyle}>
                        {Number(c.remaining_budget || 0).toFixed(2)}{" "}
                        {c.currency}
                      </td>
                      <td style={tdStyle}>
                      ...
                      <td style={tdStyle}>
                        <span style={statusBadge(c.status)}>
                          {c.status === "active" ? t("Active", "نشطة") : c.status}
                        </span>
                      </td>
                    </tr>
                  );
...
function SimpleList({ items, labelKey, valueKey, emptyText }) {
  if (!items.length) {
    return <div style={mutedText}>{emptyText}</div>;
  }
