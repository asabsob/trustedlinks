...
export default function AdminLogin() {
  const { admin, login, loading: ctxLoading } = useAdminAuth();
  ...
  // ✅ انتظر تحميل /me (لو في token محفوظ)
  if (ctxLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-sm text-gray-500">{t("Loading…", "جارٍ التحميل...")}</div>
      </div>
    );
  }
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // ما في داعي navigate هنا لأن if(admin) فوق رح يعمل redirect تلقائيًا بعد تحديث state
    } catch {
    ...
    >
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-sm p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-semibold text-green-600">Trusted Links</h1>
          <button
            type="button"
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="admin@trustedlinks.app"
              value={email}
